...
void change_line_busStops(vector<Line> &linesVec, int index) //alterei as variaveis so precisas de saber a gestao e o index da linha
{
	...
	{
		newFrequency = verifies_int(0);
		if (newFrequency == -1)
			cout << "Please enter a positive integer in minutes" << endl;

		else
		{
			timesList_temp.push_back(newFrequency);
			i++;
		} 
	}
...
} //acho que na função change_driver_maxHours o que não funciona é o método de set e get que não está a funcionar para vectors
